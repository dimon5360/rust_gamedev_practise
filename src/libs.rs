...
impl Rect {
    ...
    pub fn _print(&self) {
        println!("{} {} {} {}", self.x, self.y, self.w, self.h);
    }
...
impl Tilemap {
    ...
    pub fn new(path: &str) -> Vec<Vec<char>> {
        ...
        let mut file = File::open(path).expect(format!("can't open the file {}", path).as_str());
        let mut content = String::new();
        file.read_to_string(&mut content).expect(format!("can't read the file {}", path).as_str());
